...
SEO blacklist rules are kept in [src/config/seo.config.json](src/config/seo.config.json) and are used in two places:

- Runtime `robots` meta tags for pages that should not be indexed.
- Build-time [public/robots.txt](public/robots.txt), generated by `prebuild` together with the favicon and font assets.
- Build-time [public/sitemap.xml](public/sitemap.xml), generated by `prebuild` and automatically excluding blacklisted pages.

The current blacklist includes the callback page and the 404 fallback page.
